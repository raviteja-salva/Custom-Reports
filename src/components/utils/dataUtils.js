...
export const generateRandomData = (count) => {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      const screeningDate = faker.date.between({ from: applicationDate, to: new Date() });
      const interviewDate = faker.date.between({ from: screeningDate, to: new Date() });
      const offerDate = faker.date.between({ from: interviewDate, to: new Date() });
      const offerAcceptanceDate = faker.date.between({ from: offerDate, to: new Date() });
  
      const onboardingDate = faker.date.future({ years: 0.1, refDate: offerAcceptanceDate });
      const joinDate = faker.date.future({ years: 0.1, refDate: onboardingDate });
      ...
      return {
        ...
        interviewDates: {
          initial: interviewDate || null,
          technical: faker.date.between(interviewDate, new Date()) || null,
          hr: faker.date.between(interviewDate, new Date()) || null,
        },
        interviewers: [
        ...
        offerAcceptanceDate: offerAcceptanceDate || null,
